  ...
  class Historian
    def self.build
      ...
      if Config.tsdb
        ...
        Config.logger.info("Adding historian adapter for TSDB #{Config.tsdb_host}")
      end
      ...
      if Config.aws_cloud_watch
        ...
        Config.logger.info("Adding historian adapter for CloudWatch with AWS access key #{Config.aws_access_key_id}")
      end

      if Config.datadog
        historian.add_adapter(Historian::DataDog.new(Config.datadog_api_key, Config.datadog_application_key))
        Config.logger.info("Adding historian adapter for DataDog with api key #{Config.datadog_api_key} and application key #{Config.datadog_application_key}")
      end
    ...
    def send_data(data)
      adapters.each do |adapter|
        begin
          ...
          Config.logger.warn("Error sending data to #{adapter.class.name}: #{e.inspect} - #{e.message}")
        end
      end
    end
