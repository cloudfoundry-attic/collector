...
describe Collector::Historian::NewrelicInsights do
  describe "sending data to Newrelic Insights" do
    let(:data_threshold) {50}
    let(:time_threshold) {20}
    let(:fake_http_client) {FakeHttpClient.new}
    let(:newrelic_insights_historian) do
      Timecop.freeze(Time.at(time)) do
        described_class.new("API_KEY", 123456, fake_http_client)
      end
    end
    let(:time) { Time.now.to_i }
    let(:endpoint_url) { "https://insights-collector.newrelic.com/v1/accounts/123456/events" }
    let(:success_message) { "collector.emit-newrelic-insights.success" }
    let(:fail_message) { "collector.emit-newrelic-insights.fail" }
    let(:newrelic_insights_metric_payload) do
      {
    ...
    before do
      ...
      ::Collector::Config.stub(:newrelic_insights_data_threshold).and_return(data_threshold)
      ::Collector::Config.stub(:newrelic_insights_time_threshold_in_seconds).and_return(time_threshold)
    ...
    def submit_n_events(n)
      n.times do
        newrelic_insights_historian.send_data(newrelic_insights_metric_payload.merge(value: @counter))
        @counter += 1
      end
    end
    ...
    it "aggregates the passed in data and sends the post to data dog once it hits the threshold number of data points" do
      ...
      Timecop.freeze(Time.at(time + 1)) do
        ...
        body.length.should equal(data_threshold)
        values = body.map {|entry| entry["value"]}
    ...
    it "aggregates the passed in data and sends the post to newrelic insight after the threshold time has elapsed" do
      ::Collector::Config.logger.should_not_receive(:warn)
    ...
    it "converts the properties hash into a Newrelic Insights Event" do
      ::Collector::Config.logger.should_not_receive(:warn)
      ::Collector::Config.logger.should_receive(:info).with(success_message, number_of_metrics: 1, lag_in_seconds: 0)

      Timecop.freeze(Time.at(time + time_threshold)) do
        newrelic_insights_historian.send_data(newrelic_insights_metric_payload)
      end
    ...
    context "when the passed in data is missing a timestamp" do
      it "uses now" do
        newrelic_insights_metric_payload.delete(:timestamp)
        ::Collector::Config.logger.should_not_receive(:warn)
        ::Collector::Config.logger.should_receive(:info).with(success_message, number_of_metrics: 1, lag_in_seconds: 0)
        ...
        fake_http_client.parsed_post_body[0]["timestamp"].should == time + time_threshold
      end
    end
